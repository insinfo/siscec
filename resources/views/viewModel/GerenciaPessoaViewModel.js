...
GerenciaPessoaViewModel.prototype.getPessoasFisicas = function () {
    ...
    self.dataTablePessoaFisica = self.tablePessoaFisica.DataTable({
        'columnDefs': [{
            'targets': 0, 'searchable': false, 'orderable': false, 'width': '1%', 'className': 'dt-body-center', 'render': function (data, type, full, meta) {
            ...
            url: self.webserviceSissecBaseURL + "pessoas/datatable", type: "POST", data: {'tipoPessoa': 'fisica'}
...
GerenciaPessoaViewModel.prototype.getPessoasJuridicas = function () {
    ...
    self.dataTablePessoaJuridica = self.tablePessoaJuridica.DataTable({
        'columnDefs': [{
            ...
            url: self.webserviceSissecBaseURL + "pessoas/datatable", type: "POST", data: {'tipoPessoa': 'juridica'}
...
GerenciaPessoaViewModel.prototype.resetForm = function (tipoPessoa) {
    ...
    $('#' + self.gerenciaPessoaModalId).find(':input').not(':button, :submit,select,.select-dropdown').each(function () {
        var item = $(this);
        if (item.attr('name') === 'tipoPessoa')
        {
            item.val(tipoPessoa);
        }
        else if (item.attr('name') === 'validaoCorreios')
        {
            item.val("false");
        }
        else if (item.attr('name') === 'enderecoDivergente')
        {
            if (item.val() === 'true')
            {
                item.val('false');
...
// parte add pessoa
GerenciaPessoaViewModel.prototype.getPaises = function (select) {
    var self = this;
    self.customLoading.show();
...
GerenciaPessoaViewModel.prototype.getUFs = function (select) {
    var self = this;
    self.customLoading.show();
...
GerenciaPessoaViewModel.prototype.getMunicipios = function (idUF, select) {
    var self = this;
    self.customLoading.show();
    if (idUF == null)
    {
...
GerenciaPessoaViewModel.prototype.getCEPbyEndereco = function (endereco, refCorrentDivEndereco) {
    ...
    self.correiosDataTable = self.tableResultCorreios.DataTable({
        oLanguage: {
            "sStripClasses": "", "sSearch": "", "sSearchPlaceholder": "Digite aqui palavras-chave", "sInfo": "_START_ -_END_ de _TOTAL_", "sLengthMenu": '<span></span><select class="browser-default">' + '<option value="10">10</option>' + '<option value="20">20</option>' + '<option value="30">30</option>' + '<option value="40">40</option>' + '<option value="50">50</option>' + '<option value="-1">All</option>' + '</select></div>'
        }, bAutoWidth: false, responsive: true, processing: true, serverSide: true, ordering: false, searching: false, info: false, ajax: {
            url: self.webservicePmroBaseURL + "correiosdt", type: "POST", data: dataToSender, error: function (jqXHR, textStatus, errorThrown) {
...
GerenciaPessoaViewModel.prototype.getEnderecoByCEP = function (cep, refCorrentDivEndereco) {
    ...
    this.restClient.setWebServiceURL(this.webservicePmroBaseURL + 'correios/endereco/' + cep);
    this.restClient.setMethodGET();
    this.restClient.setSuccessCallbackFunction(function (data) {
        //set o input validaoCorreios para true
        refCorrentDivEndereco.find('[name="validaoCorreios"]').val('true');
        refCorrentDivEndereco.find('[name="cep"]').val(data['cep']);
        var selectPais = refCorrentDivEndereco.find('[name="pais"]');
        var selectPaisOption = selectPais.find(" option:contains('Brasil')");
        selectPaisOption.val(selectPaisOption.val());

        var selectUf = refCorrentDivEndereco.find('[name="uf"]');
        var selectUfOption = selectUf.find(" option:contains('" + data['uf'] + "')");

        if (selectUfOption.length > 0)
        {
            console.log('seta o estado');
            //  clearSelected(selectUf);
            selectUf.val(selectUfOption.val());
            self.updateSelect();
        }
        ...
        var timerFunc = setInterval(function () {
            if (self.customLoading.isProcessing() == false)
            {
                ...
                var selectMunicipioOption = selectMunicipio.find(" option:contains('" + data['municipio'] + "')");

                if (selectMunicipioOption.length > 0)
                {
                    console.log('seta municipio');
                    //clearSelected(selectMunicipio);
                    selectMunicipio.val(selectMunicipioOption.val());
                    self.updateSelect();
                }
            }
        }, 500);
        ...
        var selectTipoLogradouroOption = selectTipoLogradouro.find(" option:contains('" + data['tipo'] + "')");

        if (selectTipoLogradouroOption.length > 0)
        {
            selectTipoLogradouroOption.attr("selected", true);
        }
        else
        {
            selectTipoLogradouro.append($('<option>', {
                value: data['tipo'], text: data['tipo']
            }).attr("selected", true));
        }
        Materialize.updateTextFields();
        self.customLoading.hide();
...
GerenciaPessoaViewModel.prototype.incluirOrAlterarPessoa = function (action) {
    var self = this;
    ...
    this.restClient.setErrorCallbackFunction(function (jqXHR, textStatus, errorThrown) {
        ...
        alert('Mensagem: Não foi pocivel salvar os dados, verifique se você esta conectado, ou se esta pessoa já foi cadastrada!' + '\n\n' + jqXHR.responseJSON.info);
    });
    this.restClient.exec();
    return true;
};
...
GerenciaPessoaViewModel.prototype.addEndereco = function (empty, enderecoData) {
    ...
    {
        var html = $('<div id="enderecoItem' + itemCount + '" class="enderecoItem card customCard"><div class="row"><div class="input-field col s12 m2"><input name="cep" type="text" maxlength="8" minlength="8" onfocus="preventsLetter(this)"><label>CEP</label></div><div class="input-field col "><button class="waves-effect waves-light btn purple lighten-1" type="button"name="btnBuscarEndereco">Preencher</button>   <button class="waves-effect waves-light btn purple lighten-1 " type="button"name="btnShowModalBuscaCEP">Buscar CEP</button></div></div><div class="row"><div class="input-field col s12 m3"><select name="tipoEndereco"><option selected disabled>Tipo de Endereço</option><option value="Residencial">Residencial</option><option value="Comercial">Comercial</option></select><label></label></div><div class="input-field col s12 m3"><select name="pais"><option value="" selected disabled>Pais</option></select><label></label></div><div class="input-field col s12 m3"><select name="uf"><option value="" selected disabled>Estado</option><option value="">Rio de Janeiro</option><option value="">Brasilia</option></select><label></label></div><div class="input-field col s12 m3"><select name="municipio"><option value="" selected disabled>Municipio</option></select><label></label></div><div class="input-field col s12 m3"><input name="bairro" type="text" maxlength="70" onfocus=\'preventsNumber(this)\'><label>Bairro</label></div><div class="input-field col s12 m2"><select name="tipoLogradouro"><option value="" selected disabled>Tipo de Logradouro</option><option value="Rua">Rua</option><option value="Avenida">Avenida</option><option value="Beco">Beco</option><option value="Estrada">Estrada</option><option value="Praça">Praça</option><option value="Rodovia">Rodovia</option><option value="Travessa">Travessa</option><option value="Largo">Largo</option></select><label></label></div><div class="input-field col s12 m3"><input type="text"  name="logradouro"><label>Logradouro</label></div><div class="input-field col s12 m1"><input  name="numeroLogradouro" type="text" maxlength="10"   onfocus=\'preventsLetter(this)\'><label>Número</label></div><div class="input-field col s12 m3"><input type="text"  name="complemento" maxlength="70"><label>Complemento</label></div><input type="hidden" name="validaoCorreios" value="false"/></div><div class="row"><div class="col"><input id="enderecoDivergente" name="enderecoDivergente" value="false"   onclick="$(this).val(this.checked ? \'true\' : \'false\')" type="checkbox"><label for="enderecoDivergente">Marque aqui caso o endereço dos correios seja divergente</label></div></div></div>');

        html = self.enderecoBlock.append(html);
        self.getPaises();
        self.getUFs();
        self.getMunicipios();
        ...
        if (enderecoData)
        {
            ...
            html.find('[name="pais"]').val(enderecoData['idPais']);
            html.find('[name="uf"]').val(enderecoData['idUf']);
            html.find('[name="municipio"]').val(enderecoData['idMunicipio']);
            ...
            html.find('[name="enderecoDivergente"]').val(enderecoData['divergente']);
            html.find('[name="validaoCorreios"]').val(enderecoData['validacao']);
        }
        Materialize.updateTextFields();
        self.updateSelect();
    }
...
GerenciaPessoaViewModel.prototype.eventos = function () {
    ...
    $(document).on('change', '[name="' + self.selectUfName + '"]', function (e) {
        ...
        self.getMunicipios(selectUF.val(), selectMunicipio);
    });

    //quando selecionar um estado de UfBuscaCEP obtem os municipios deste estado
    self.selectUfBuscaCEP.change(function (e) {
        self.getMunicipios(self.selectUfBuscaCEP.val(), self.selectMunicipioBuscaCEP);
    });
    ...
    self.btnSalvar.click(function (event) {
        self.incluirOrAlterarPessoa(self.actionIncluirOrEditarPessoa);
    });
