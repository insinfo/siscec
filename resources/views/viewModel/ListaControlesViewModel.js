    ...
    self.restClient.setSuccessCallbackFunction(function (setoresData) {
        ...
        self.treeViewModalBuscaSetor.treeview({
            data: setoresData, onNodeSelected: function (event, node) {
                ...
                correnteInput.attr('data-content', node['idSetor']);
...
ListaControlesViewModel.prototype.getControles = function () {
    ...
    var filtros = {
        "idSetor": self.inputSetorFiltro.attr('data-content'), "modalidade": self.selecModalidadeFiltro.val(), "status": self.selectStatusFiltro.val(), "search": self.inputSearchFiltro.val(), "order": self.selectOrderFiltro.val(), "start": offset, "length": self._itemsPerPage, "draw": 1
    };
    //console.log(filtros);
    self.customLoading.show();
...
ListaControlesViewModel.prototype.displayControle = function (data) {
    ...
    this.inputEditSetor.attr('data-content', data['idSetor']);
...
ListaControlesViewModel.prototype.getPessoas = function (inputDOM, tipoPessoa) {
    ...
    self.customLoading.show();
    self.restClient.setWebServiceURL(this.webservicePmroBaseURL + "pessoas/datatable");
    self.restClient.setMethodPOST();
    self.restClient.setDataToSender({
        'tipoPessoa': tipoPessoa, 'draw': '1', 'start': '0', 'length': '1', 'search': ''
    });
    self.restClient.setSuccessCallbackFunction(function (data) {
        self.customLoading.hide();
        ...
        self.dataTablesPessoaJuridica = self.tablePessoaJuridica.DataTable({
            order: [[1, 'asc']], oLanguage: {
                ...
                url: self.webservicePmroBaseURL + "pessoas", type: "POST", data: {'tipoPessoa': tipoPessoa}
...
ListaControlesViewModel.prototype.getMedicoesPagas = function (dataToSend) {
    ...
    self.dataTableMedicoesPagas = self.tableMedicoesPagas.DataTable({
        info: false, bPaginate: true, bAutoWidth: false, responsive: true, processing: true, serverSide: true, ordering: false, searching: false, ajax: {
            url: self.webserviceSiscecBaseURL + "medicoes/pagas", type: "POST", data: dataToSend
        }, columns: [{
            ...
            "data": "valor", "render": function (data, type, row) {
                return '<span class="' + row['cor'] + '">' + data + '</span>';
            }
        }], oLanguage: {
...
ListaControlesViewModel.prototype.saveControle = function () {
    ...
    var dataToSender = {
        'numeroControle': self.inputEditNumeroControle.val(), 'idSetor': self.inputEditSetor.attr('data-content'), 'modalidade': self.selectEditModalidade.val(), 'idPessoa': self.inputEditEmpresa.attr('data-content'), 'previsaoGasto': self.inputEditPrevisaoGasto.val(), 'objeto': self.textAreaEditObjeto.val(), "concluido": false
    };
